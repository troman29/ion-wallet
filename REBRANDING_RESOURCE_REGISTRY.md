...
| Runtime API, RPC, indexer, static CDN, TonConnect bridge | `src/config.ts`, `webpack.config.ts` | требует решения | Для каждого endpoint определить владельца, production URL, мониторинг и контракт. Отдельно закрыть отсутствие ION-совместимого v3 indexer. |
...
| ION Connect registry | manifest / metadata кошелька и реестр ION Connect | требует регистрации | Создать официальный wallet entry: название, `ion://`/`ion-tc://`, сайт, иконки, ссылки на store и технический контакт. Проверить подключение dApp после публикации. |
...
4. Сверить store/registry metadata с package IDs: `io.ice.wallet`, iOS IONWallet targets, `ion://`, `ion-tc://`, `ion-wc://`.
